---
description: Implement tasks from an OpenSpec change, ticking them off in tasks.md as you go
argument-hint: <change-name or blank to infer>
---
...
3. **Get apply instructions**
   ...
   **Handle states:**
   - If `state: "blocked"` (missing artifacts): show message, suggest using `/openspec:propose` to finish artifacts
...
7. **On completion or pause, show status**
   ...
   - If all done: suggest archive (`/openspec:archive`)
...
- For this project: `cargo test` after meaningful changes; preserve the cached-per-version Markdown invariants in `src/model.rs` / `src/lib.rs`
