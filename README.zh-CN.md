...
- **Git 同步**可将普通笔记仓库绑定到一个上游分支。“立即同步”会保存策略允许的笔记和附件，以确定性消息提交，安全地抓取并合并，然后把一个明确的提交推送到远端。设置、认证、冲突与恢复说明见 [Git 同步指南](docs/git-sync.md)。
...
- 应用内偏好设置面板在**通用**中可配置语言、侧边栏显示、预览自适应宽度、专注/打字机模式、代码行号、同步滚动、显示隐藏文件和标题菜单层级；在**外观**中可配置主题、按平面的字体（源码/阅读/代码，默认跟随主题）、字号和段落间距。
