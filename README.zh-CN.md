...
- **可视化编辑** — 面向所见即所得、基于源码的编辑界面。正文保持渲染并渐进显示必要语法；普通围栏代码正文、块级数学公式、行内图片字段与 GFM 表格单元格提供精确的直接编辑器。HTML、YAML 前言、已注册图表、格式错误及其他含义不明确的结构保留完整、保守的源码编辑区。斜杠命令面板与紧凑的右键块级菜单提供精确的块级转换（段落、标题、列表、引用、围栏代码、分隔线、表格）、复制、上移/下移、源码安全的拖拽重排与删除；选区上下文格式工具栏与可视化链接编辑器对每次操作执行一次精确的源码变更。它不是独立的富文本文档模型——底层 Markdown 始终是唯一数据源。
...
- 应用内偏好设置面板可配置主题、语言、侧边栏显示、预览自适应宽度、专注/打字机模式、代码行号、同步滚动、显示隐藏文件和标题菜单层级。
...
- 可视化编辑以尽可能接近所见即所得为目标，同时保留标准 Markdown；不支持、格式错误或字节映射含义不明确的结构会有意显示精确源码，而不会猜测富文本树变更；仅当可证明存在不重叠的源码边界时才提供块级重排。
