...
`v0.4.1..v0.4.3` (and `v0.3.12..v0.4.3`) contains no changes under
...
rich-paste evidence carry over to v0.4.3. The `v0.4.2` tag was pushed but its
Linux job failed before publication (Ubuntu `rpm-build` package name), and no
GitHub Release was created for it. Before tagging v0.4.3 on 2026-09-23, the
source-tree `verify-bundle` command passed on Windows at pinned MarkNice
revision `c009c1ec7e7c92f89afa5a32edcb126b5296bda7`. v0.4.3 additionally ships
