...
| Emphasis, strong, strike, inline code, links, highlight, super/subscript, inline math, backslash-escaped ASCII punctuation, decoded HTML entity references (proven single-codepoint set), supported inline HTML (`em`/`i`, `strong`/`b`, `s`/`del`/`strike`, `code`, `mark`, `sub`, `sup`, `br`) | Rendered with progressive source reveal; navigation icons and atoms stay on the logical line that owns the construct; escapes hide the backslash byte, entity runs render the decoded character while the complete authored `&…;` token stays hidden until reveal, and HTML tags stay hidden markers whose styles compose with Markdown formatting (`<br>` is an atomic line-break run) | Smallest complete proven syntax group (`\X` pair, complete `&…;` entity token, or full open-tag…close-tag element) | Malformed, overlapping, or ambiguous syntax; entity references outside the proven decode table; escapes or inline HTML outside the byte-proven subset (uncovered-`\`-gap + punctuation proof for escapes; reconstruction proof against the parser's decode rules for entities; exact unattributed tags with strict stack pairing for HTML) | Reveal-group containment, caret affinity, cross-run selection, UTF-8 input, mixed-layout line stacking, escape gap-splitting, entity decode-table parity with the parser, HTML pairing/demotion, `<br>` atomic boundaries |
...
| Inline Markdown image | Bounded preview, caption, missing-resource placeholder, and focused replace/width/alignment controls. A paragraph or heading that contains nested `![...](...)` (including adjacent lines with no blank line) is partitioned into disjoint visual rows: prose before the image, the image row, and leftover prose after | Complete authored inline-image source for each image row; each prose slice owns only its source range | Reference, angle-bracket destination, multiline, malformed form, or stale/ambiguous exact-image target; list-item inline Markdown images keep the conservative overlapping-island fallback | Source coverage, mixed-paragraph partition (no-blank-line, same-line, multiple images), quoted paragraph leaves, blank-line-separated and image-only cases, caption and presentation, broken/local/remote image, exact replacement, UTF-8, and one-edit undo |
...
| Inline raw-HTML `<img>` in prose | Rendered image atom in the inline flow (same loader as preview: local/remote/data-URI) with progressive source reveal of the complete tag | Complete byte-exact `<img …>` tag | Unsupported inline HTML in the same block (unknown or attributed tags, stray/unclosed pairing), a non-tag/partial/multi-tag slice, missing `src`, or table-cell context (cells keep flattened alt/URL text, matching Read mode) | Exact tag ranges, reveal-on-focus/restore-on-blur, no whole-block island, claim/preload/eviction parity with block-level images |
...
| Other inline HTML and YAML front matter | Complete source-backed island | Complete authored block | Always (inline HTML outside the supported subset listed above, and any tag the exact recognizer rejects) | Exact source preservation, preview/export behavior |
...
Prioritized open gaps (refreshed 2026-08-28; each closes via a future change citing the `WYSIWYG coverage roadmap` requirement):
...
| 6 | Unsupported inline-HTML forms (unknown/attributed/stray tags) | Verbatim tag fragments unfocused; island focused | Progressive-reveal or inert atoms | Medium | `src/visual.rs:1732-1821` |
| 7 | Task-list checkbox click | Glyph not interactive | Rendered (click toggles `[ ]`/`[x]`) | Small | `src/app/preview.rs:2919-2944` |
| 8 | Angle-bracket autolinks (`<https://…>`) | Whole paragraph island (link reveal requires `[`-shaped sources) | Progressive-reveal | Small | `src/visual.rs:2276-2278`, invalidation `:2202-2205`; no dedicated test yet |
...
| 10 | Empty list items (`- `) | Dropped by the parser → Unsupported gap island | Rendered (editable empty row) | Small | `src/parse.rs:112-126`, `src/lib.rs:1280-1283` |
...
| 12 | Residual gap bytes between known blocks | Unsupported island (catch-all) | Closed construct-by-construct | — | `src/visual.rs:857-892` |
| 13 (secondary) | Entity references outside the proven decode table (multi-codepoint names, unmaintained names, invalid numeric forms) | Conservative fallback keeps the containing paragraph source-backed | Progressive-reveal | Data-only (extend `NAMED_ENTITY_DECODES`; multi-codepoint forms need multi-char decoded spans) | `src/visual.rs::NAMED_ENTITY_DECODES` + `decode_entity_token`; parity test loop pins every entry to parser output |
