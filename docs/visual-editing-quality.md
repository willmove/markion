...
| Inline Markdown image | Bounded preview plus read-only caption (Title, or Alt Text when no Title); source-backed island | Complete authored image source | Reference, angle-bracket destination, multiline, malformed form, or any inline image (direct field editing removed) | Source coverage, caption presentation, broken/local/remote image, no guessed mutation |
