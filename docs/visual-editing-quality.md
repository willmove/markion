...
| Emphasis, strong, strike, inline code, links, highlight, super/subscript, inline math, backslash-escaped ASCII punctuation, supported inline HTML (`em`/`i`, `strong`/`b`, `s`/`del`/`strike`, `code`, `mark`, `sub`, `sup`, `br`) | Rendered with progressive source reveal; navigation icons and atoms stay on the logical line that owns the construct; escapes hide the backslash byte and HTML tags stay hidden markers whose styles compose with Markdown formatting (`<br>` is an atomic line-break run) | Smallest complete proven syntax group (`\X` pair or full open-tag…close-tag element) | Malformed, overlapping, or ambiguous syntax; HTML entities, escapes or inline HTML outside the byte-proven subset (uncovered-`\`-gap + punctuation proof for escapes; exact unattributed tags with strict stack pairing for HTML) | Reveal-group containment, caret affinity, cross-run selection, UTF-8 input, mixed-layout line stacking, escape gap-splitting, HTML pairing/demotion, `<br>` atomic boundaries |
