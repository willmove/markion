...
| Display/fenced block math | `VisualBlockEditor::Math`: rendered formula plus LaTeX payload editor; Pending/Error keep that editor with the payload forced open until Ready | LaTeX payload only; delimiters remain authored; fallback payload is the complete authored span when the delimiter split fails | Inline-only/ambiguous form | Valid/invalid/pending render, delimiter preservation, CJK/emoji IME geometry, one-action undo, no source-island fallback on Pending/Error |
| Inline Markdown image | Image-only paragraphs keep the bounded preview, caption, missing-resource placeholder, and focused replace/width/alignment controls (integer 10–100% plus a drag-resize handle on exactly mapped inline images). A paragraph, heading, quote, or list item that **mixes** `![...](...)` with other prose (including leading same-line `![img](url)trailing` and `text ![img](url) more`) keeps the image as an inline atom on the same visual line as adjacent text, matching Read / Split Preview; focused caret reveals the authored `![…](…)` bytes. A block-level image with a proven exact span — inline `![alt](url)` or single-line reference `![alt][label]` / `![alt][]` / shortcut `![alt]` — additionally carries the collapsible-source chrome: hover `</>` expands one monospaced payload over the complete authored span above the image, and an unloadable destination forces the payload visible; data-URI payloads render as one atomic, chip-styled size-labeled summary token (`…{size}…`) that edits as a single unit while the structural syntax stays verbatim — the same shared token covers raw-HTML block payloads, source-island fallbacks, focused table cells, caret-revealed inline image/`<img>` runs, and revealed link destinations. Width/alignment controls stay gated on `inline_image_at` (inline destinations only) so reference spans are not rewritten to `![alt](url)` | Complete authored image source for a standalone image row (inline or single-line reference); mixed atoms own the `![…](…)` bytes inside the parent prose row | Angle-bracket-only destination, multiline, malformed form, or stale/ambiguous exact-image target on a standalone image-only row (such rows show no source toggle and keep the island fallback) | Source coverage, mixed inline atoms (no-blank-line, same-line, leading same-line, multiple images), quoted/heading/list-item leaves, blank-line-separated and image-only cases, caption and presentation, broken/local/remote image, whole-span payload edit as one exact replacement, forced expand on load failure, UTF-8, and one-edit undo; data-URI elision on every surface via the shared scanner (token boundaries, atomic replacement/deletion, fingerprint-based forced expand, collapsed-frame zero span work, HTML/inline/island/table-cell coverage); reference-style block images keep the picture when focused |
| Contextual inline formatting and links | Selection toolbar for bold, italic, inline code, and links; focused URL/title link editor. A non-empty selection fully inside one Visual Edit table cell uses the same context-menu group | One non-empty selection inside one proven editable inline run, one complete exact inline-link source, or one table cell source range | Cross-run selection, conservative/malformed syntax, stale document version/range, reference-style link, or a selection that crosses a `|` / cell boundary | Presentation-only version stability, exact UTF-8 range, URL/title escaping, confirm/cancel, IME input, one-edit undo/redo, and in-cell Bold wrap |
...
| GFM table | `VisualBlockEditor::Table`: editable header/body cells plus an interaction-gated row/column toolbar (hover or caret in the table) with structural boundary disabling and whole-table delete through exact block delete. Ragged rows stay a focused grid: short rows pad empty insertion ranges at the line's content end; extra cells remain editable. Adjacent-column drag handles persist widths as `<!-- markion-cols:N,N -->` immediately before the table (absorbed into the table source range). A selection fully inside one cell exposes Bold/Italic/Inline Code/Link in the block context menu | Logical cell owned by the canonical caret for row/column edits; one deterministic full-table replacement for width reflow; whole-table delete uses the complete newline-safe source unit including an absorbed column-width comment | Ambiguous/unparseable table, stale toolbar target, a toolbar whose table does not own the caret, or nested/quoted tables whose block delete is unsupported | Escaped-pipe projection, UTF-8 width reflow, alignment/export preservation, traversal, hover/caret toolbar visibility without version changes, toolbar cell ownership and boundary availability, whole-table delete undo, multi-table isolation, IME, one-edit undo/redo history, ragged-row padding, column-drag overlay without version change, and in-cell selection format |
...
| In-document `[TOC]` / `[toc]` | Live nested outline from the per-version heading cache; Visual Edit reveals the authored token above the list while the block owns the caret | Exact `[TOC]` paragraph range; clicks jump via outline navigation and must not bump document version | List-item `[TOC]`, fenced code, or other non-paragraph constructs stay ordinary text | Derivation (no leftover paragraph), list-item unchanged, GPUI TOC click without mutation |
| Footnote reference hover | Presentation-only tooltip with the matching definition text in Read, Split Preview, and Visual Edit; Visual Edit keeps the existing jump icon | None (hover is not an edit); click-to-jump still uses the definition block range | Unresolved labels omit the tooltip | Tooltip wiring without version change; unresolved labels have no tooltip |
| In-document `#fragment` / `{#id}` heading links | Bare `#anchor` destinations jump like the outline panel (Unicode-preserving slugs, authored `{#id}` wins, `hello` / `hello-1` uniquify); Visual Edit shows a `#` navigation icon | None for a successful jump; unmatched fragments keep URL-open behavior | HTML-block `<a href="#">`, cross-file `other.md#heading` | CJK titles, authored ids, duplicate titles, GPUI `#hello` click without mutation |
| Blank lines and trailing whitespace | Stable empty-paragraph Visual Edit row: body line-height content/caret geometry, with paragraph spacing contributed once at the end of each same-context paragraph/whitespace body flow; click and Up/Down enter the existing source range; focused thin caret, not a source island. Click is caret placement only (no inserted newline) | Exact whitespace range | N/A | Complete source coverage; gap click/arrow without version change; typing between headings or paragraphs without moving following content; custom paragraph-spacing reflow; no pointer-created phantom edits |
...
1. **Rendered WYSIWYG** — shown in its rendered form. Dedicated field/payload editors (fenced code payload plus the hover/caret-revealed info-token chip, block math including Pending/Error, diagrams, Markdown and inline-HTML images with the collapsible whole-span source toggle above the image — including single-line reference-style block images, GFM tables with cell editors including ragged rows, YAML front matter via `VisualBlockEditor::FrontMatter`, HTML blocks via `VisualBlockEditor::Html`) are the rendered form of their constructs. Covers prose rows, headings (including empty ATX headings), lists/task items (including empty items), blockquote flows, GFM alerts, horizontal rules, whitespace rows, footnote definitions/references (including hover preview of the definition), in-document `[TOC]` widgets, `#fragment` heading jumps, link reference definitions, and HTML blocks (shared pipeline plus collapsible source payload).
...
Prioritized open gaps (refreshed 2026-09-12; each closes via a future change citing the `WYSIWYG coverage roadmap` requirement):
...
| 4 | Multiline/malformed images | Renders unfocused when the parser classifies them; focused → island | Rendered | Medium | `src/inline_edit.rs` (`proven_single_line_image_span`); angle-bracket-only and multiline forms still refuse the editor |
...
Closed in `close-visual-edit-high-freq-gaps`: YAML `---` front matter (former gap 1) is a collapsible document header; single-line reference-style block images (former gap 4 reference path) keep the picture and whole-span payload when focused; ragged GFM tables (former gap 5) keep padded cell editors; display/fenced math Pending/Error (former gap 11) stays on the math payload editor.

Closed in `add-visual-toc-footnotes-anchors`: in-document `[TOC]` (former Typora-muscle-memory gap) renders a live outline; footnote references show a definition tooltip on hover; bare `#fragment` links and authored `{#id}` heading ids jump in-document instead of calling the platform URL opener.
