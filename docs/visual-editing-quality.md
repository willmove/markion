...
| Inline Markdown image | Bounded preview, caption, missing-resource placeholder, and focused replace/width/alignment controls | Complete authored inline-image source; replacement serializes one proven range and stores presentation metadata in the title | Reference, angle-bracket destination, multiline, malformed form, or stale/ambiguous exact-image target | Source coverage, managed paste/drop resources, caption and presentation, broken/local/remote image, exact replacement, UTF-8, and one-edit undo |
