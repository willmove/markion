...
- Present these onboarding routes as a short notes-repository setup: connect the open folder when possible, otherwise clone a remote or initialize the open notes folder, with `main` and a destination derived from the remote as editable defaults. Keep branch, scope, and transport details in an advanced review instead of requiring Git vocabulary up front.
