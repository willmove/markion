...
The first-use surface SHALL present every Backup and Sync route as one directly visible selectable set and SHALL preselect the shortest applicable route from detected context, instead of burying alternatives behind a disclosure. A suitable repository whose root is the notes workspace SHALL be the preselected route; an ordinary notes folder SHALL lead with enabling backup and sync; an empty/new location SHALL lead with obtaining notes from an existing sync address. The selected route SHALL be marked so the recommendation stays visible. The surface SHALL use notes folder, sync address, and backup-and-sync language; ask only for fields required by the selected route; derive an editable destination from the address; explain where an address can be obtained; suggest `main` only when no branch is discovered; and place branch, remote, author, scope, existing staging, transport, and history details in review/advanced controls. Ordinary configured use SHALL return directly to Sync Now.
...
- **THEN** the app presents every Backup and Sync route directly selectable with the contextually recommended route already preselected and marked
