...
File paths shown to the user, written to the clipboard, or persisted in session state SHALL be in platform-normal form — on Windows, drive-rooted paths SHALL NOT carry the extended-length verbatim prefix (`\\?\`), and network paths SHALL NOT carry the `\\?\UNC\` form (they use `\\server\share\...`). Paths attached to opened content (document tabs, image tabs), the workspace root, file-tree entries, recent-files entries, recent-workspace roots, and per-workspace snapshot tab paths SHALL all be in normal form, so that any surface reading them — copy actions, status feedback, reveal-in-file-manager, session persistence, the recent-workspace switcher — presents the normal form. A path that genuinely requires the extended-length syntax (e.g. longer than the classic Windows path limit) MAY retain the verbatim form, since correct file access takes precedence over cosmetic presentation.
...
- **WHEN** a session persisted by an earlier version contains `\\?\`-prefixed open-file, active-file, workspace-root, recent-file, recent-workspace, or per-workspace snapshot entries
