...
#### Scenario: Path-backed tabs restore with the workspace
- **WHEN** the editor restores a workspace snapshot on launch or on an explicit workspace switch
- **THEN** the recorded path-backed tabs reopen in the recorded order through the existing open/tab APIs
- **AND** untitled tabs are not recreated from the snapshot
- **AND** cursor, selection, scroll, and undo history start fresh for each newly opened restored tab

#### Scenario: Workspace switch keeps dirty tabs
- **WHEN** the user switches workspace and a document tab has unsaved changes
- **THEN** that tab stays open with its text, dirty flag, undo history, and recovery snapshot
- **AND** Save / Don't Save / Cancel is not shown until the user closes that tab or quits
- **AND** an already-open dirty tab for a restored snapshot path is reused instead of being reloaded from disk
