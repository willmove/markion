...
- A new `WYSIWYG coverage roadmap` requirement is added to `markdown-editing`, enumerating the known WYSIWYG gaps and committing that each SHALL be closed by a future change. The roadmap is the source of truth for prioritization. It was **refreshed on 2026-08-19** against the implementation: escaped punctuation, the supported inline-HTML subset, standalone HTML blocks, smart-punctuation substitution, reference-style links, inline-dollar math, footnote/link-reference definitions, heading attributes, and GFM alerts were closed by interim changes and are no longer listed; the remaining primary gaps are decoded HTML entities in prose, frontmatter, and indented code blocks, plus secondary gaps (unclosed fences, reference-style images, malformed tables, unsupported inline-HTML forms, angle-bracket autolinks, task-list checkbox click, definition lists, empty list items, math render-failure states).
...
- **Future work**: each remaining roadmap gap becomes a candidate for a follow-up change (e.g. `fix-visual-edit-entity-projection` for decoded entities, `render-frontmatter-form-in-visual-edit`, `render-indented-code-in-visual-edit`, `fix-visual-edit-autolink-reveal`, `enable-visual-edit-definition-lists`, `fix-visual-edit-empty-list-items`, `make-task-checkbox-clickable`, etc.). Those changes will cite this roadmap requirement as their motivation. Gaps from the original 2026-07-21 draft that were already closed (`render-visual-edit-escapes-and-inline-html`, `render-visual-edit-html-images`, `resolve-reference-links-in-visual-edit`, HTML-block rendering in `6220b33`) are removed from the roadmap rather than re-listed.
