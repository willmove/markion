...
- [ ] 7.5 Manually verify: New File / New Folder / Rename open the editor in place of / below the target row, typing replaces the pre-selected name (extension preserved on rename), arrows/Home/End/Shift+arrows/Backspace/Delete edit only the name buffer, Enter commits, clicking elsewhere commits without opening the clicked file, Escape and opening a menu cancel, deleting a non-empty folder asks twice and removes recursively, deleting a file/empty folder asks once.

## 8. Inline name editor upgrade (caret, selection, click-away commit)

- [x] 8.1 Extend `PendingNameInput` with `cursor` / `anchor` byte offsets (clamped to char boundaries). `PendingNameInput::new` pre-selects the base name for Rename (extension preserved, dotfiles whole, via `base_name_len`) and the whole prefilled name for create kinds.
- [x] 8.2 Make the redirected input caret-aware while a name is pending: `insert_redirected_text` replaces the active IME composition and selection at the caret; backspace (`pop_name_text`) removes composition, else selection, else the char before the caret; `delete_name_text_forward` is the Delete-key counterpart; `NameCaretMove` (Left/Right/Home/End/SelectLeft/SelectRight/SelectAll) drives `move_name_caret`.
- [x] 8.3 Route the Left/Right/Home/End/SelectLeft/SelectRight/SelectAll/Delete action handlers into the name editor while it is open so those keys never reach the document caret or selection.
- [x] 8.4 Render the editor in place of the renamed row and directly after the parent folder row (extract `file_tree_entry_row` / `file_tree_rows`); fall back to a panel-top editor row when the target row is not visible; keep the labeled prompt under the tab bar when the Files panel is hidden; show a visible caret / selection highlight and support click-to-position (Shift+click extends).
- [x] 8.5 Click-away commit: a left mouse-down below the menu bar commits through `confirm_pending_name` and sets `name_editor_click_away`, consumed by the tree-row and tab-strip mouse-up handlers so the committing click does not open a file or switch tabs; a refused (empty-name) commit keeps the editor open; opening a context menu / toggling the menu bar still cancels; while the editor is open, clicks in the editor pane leave the document caret and selection untouched.
- [x] 8.6 Tests: prefill/initial-selection contracts (incl. multi-dot and dotfile names), char-boundary helpers, document stays inert on editor-pane click, click-away commit + mouse-up guard, refused commit keeps the editor open, selection replace + arrow keys stay in the buffer, IME composition over the selection, Escape cancels without touching the filesystem.
- [x] 8.7 Run `cargo fmt` and `cargo test` (root package) — all green.
