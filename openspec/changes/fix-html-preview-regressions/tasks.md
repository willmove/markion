...
- [x] 1.1 Update the existing offset-aware preview derivation pass to recognize HTML-only parser paragraphs and emit one source-faithful `PreviewBlock::Html` without changing per-document-version cache behavior.
...
- [x] 2.1 Carry collapsible whitespace across `HtmlPreviewBuilder` text-node and inline-tag boundaries while retaining the originating style and link span metadata.
