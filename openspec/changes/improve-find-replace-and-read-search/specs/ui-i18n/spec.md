...
The system SHALL route every user-visible UI string (menu bar titles and items, in-app dropdown labels, status bar text, dialog text, search panel labels, external search-field identity and guidance, search option and navigation tooltips, accessibility labels, search validation and empty-state feedback, Read-mode replacement guidance, file tree labels, file tree context-menu labels, the file-tree create/rename inline name-prompt label and placeholder, the recursive-folder-delete confirm dialog title and detail, preferences panel labels, and the keyboard-shortcut reference) through the i18n module's `t` / `tf` / `shortcut_reference` / `sidebar_tab_label` functions. Hard-coded user-visible English literals in these surfaces SHALL NOT remain.
...
#### Scenario: Search overlay chrome reflects the active language
- **WHEN** the Find / Replace overlay is visible and the active interface language changes
- **THEN** external field identity, navigation and option tooltips, accessibility labels, current/total or empty-state feedback, regular-expression errors, replacement actions, close guidance, and Read-mode replacement guidance render in the active language through the i18n layer
- **AND** no user-visible search status or validation text is hard-coded in English
