...
- **THEN** with the open-in-current-tab preference on, the active tab's content is replaced after applying a dirty guard when that tab contains an editable document, rather than spawning a new tab
- **AND** with the preference off, a new tab is appended and activated without a dirty guard
...
- **WHEN** the open-in-current-tab preference is on, the active tab is an image tab, the untitled document, or a clean saved document, and the user opens another supported file from the file tree, a drag-drop, or Open Recent
...
- **WHEN** the open-in-current-tab preference is on, the active editable document tab is dirty, and the user opens another supported file from the file tree, a drag-drop, or Open Recent
