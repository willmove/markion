...
#### Scenario: Linux download follows the distribution family
- **WHEN** a newer Release is available on Linux x86_64
- **THEN** a pacman-based system — an `os-release` whose `ID` or `ID_LIKE` contains `arch` — is offered the `.AppImage` asset URL
- **AND** every other Linux system is offered the `.deb` asset URL
- **AND** no file is installed automatically
