...
- **WHEN** the user selects a supported image through File → Open
- **THEN** with the open-in-current-tab preference on and a replaceable active tab, the active tab is replaced by a read-only image tab for the selected path; with the preference off, or with a dirty active document tab, a new image tab is appended and activated
- **AND** the image bytes are not interpreted as document text
- **AND** a dirty active document is not prompted to discard and is not replaced
