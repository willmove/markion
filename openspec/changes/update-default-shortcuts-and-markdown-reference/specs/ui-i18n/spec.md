...
The system SHALL route every user-visible UI string (menu bar titles and items, in-app dropdown labels, status bar text, dialog text, search panel labels, file tree labels, file tree context-menu labels, the file-tree create/rename inline name-prompt label and placeholder, the recursive-folder-delete confirm dialog title and detail, preferences panel labels, the keyboard-shortcut reference, and the Markdown Reference overlay) through the i18n module's `t` / `tf` / `shortcut_reference` / `markdown_reference` / `sidebar_tab_label` functions. Hard-coded user-visible English literals in these surfaces SHALL NOT remain.
...
#### Scenario: Overlay chrome and body follow the active language
- **WHEN** the Markdown Reference overlay is opened after the interface language changes
- **THEN** its title, close control, status feedback, section titles, examples, and captions render in the active language
- **AND** every supported language returns the same non-empty section set

#### Scenario: Shortcut catalog lists the new defaults
- **WHEN** the user opens the in-app shortcut reference in any supported language
- **THEN** New Tab, Edit / source mode, Split Preview, Read mode, and Markdown Reference appear with their effective platform bindings
