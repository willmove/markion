...
#### Scenario: Diagram fence language token is editable while the payload is mounted
- **WHEN** a diagram fence's source payload editor is mounted in Visual Edit and the user clicks and edits the language label in its payload-editor header
- **THEN** the change applies as one exact replacement of only the first info-string token through the normal document mutation path
- **AND** sanitization prevents whitespace, backticks, or line breaks from entering the source through that field

#### Scenario: Retyping away from a diagram backend simply re-parses
- **WHEN** the user changes a `mermaid` fence's language token to an ordinary language identifier
- **THEN** the block re-parses and is presented as an ordinary code fence on the next derivation
- **AND** no diagram presentation state rewrites the source or suppresses the transition
