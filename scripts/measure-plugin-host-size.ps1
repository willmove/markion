param(
    ...
    [string]$ManifestPath = (Join-Path $PSScriptRoot "..\config\plugin-platform.json")
)
...
$manifestFile = Resolve-RequiredFile -Path $ManifestPath -Label 'Plugin platform manifest'
$manifest = Get-Content -Raw -LiteralPath $manifestFile.FullName | ConvertFrom-Json
if ($manifest.schema_version -ne 1 -or
    $manifest.bytes_per_mib -ne 1048576 -or
    $manifest.host_package_delta_limit_bytes -ne 1048576 -or
    $manifest.host_installed_delta_limit_bytes -ne 2097152 -or
    $manifest.pdf_plugin_archive_limit_bytes -ne 6291456 -or
    $manifest.pdf_plugin_installed_limit_bytes -ne 10485760) {
    throw 'Plugin platform size budgets do not match the approved OpenSpec change.'
}
$targetMatches = @($manifest.targets | Where-Object { $_.triple -ceq $TargetTriple })
if ($targetMatches.Count -ne 1) { throw "Target is not present exactly once: $TargetTriple" }
...
$packagePassed = $packageDelta -le [long]$manifest.host_package_delta_limit_bytes
$payloadPassed = $payloadDelta -le [long]$manifest.host_installed_delta_limit_bytes
$report = [ordered]@{
    schema_version = 1
    control_revision = $manifest.host_control_revision
    target = $TargetTriple
    format = $Format
    byte_unit = 'bytes'
    ...
    package = [ordered]@{
        ...
        limit_bytes = [long]$manifest.host_package_delta_limit_bytes
    ...
    installed_payload = [ordered]@{
        ...
        limit_bytes = [long]$manifest.host_installed_delta_limit_bytes
...
if (-not $report.passed) {
    throw "Plugin host size budget exceeded for $TargetTriple/$Format; report retained at $resolvedOutput"
}
