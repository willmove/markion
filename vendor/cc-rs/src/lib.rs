...
impl Build {
    ...
    fn compile_objects(&self, objs: &[Object]) -> Result<(), Error> {
        ...
        #[cfg(feature = "parallel")]
        // Markion: MSVC 14.44 intermittently raises C1056 when cc-rs launches
        // several object compilations at once. Keep the normal parallel path
        // everywhere else and use the sequential loop below for MSVC only.
        if objs.len() > 1 && self.get_target()?.env != "msvc" {
    ...
    fn add_default_flags(
        ...
        match cmd.family {
            ...
            ToolFamily::Msvc { clang_cl } => {
                ...
                // files.
                // Markion: cl.exe 19.44 intermittently fails while rewriting
                // that timestamp field (C1056), including in a sequential
                // build. clang-cl does not exhibit the MSVC object-writer bug.
                if clang_cl {
